...
from dual_mobilenet import DualMobileNet
...
class DeepLab(nn.Module):
    def __init__(self, out_planes, criterion, norm_layer, pretrained_model=None):
        ...
        self.backbone = DualMobileNet()
        self.dilate = 2

        for m in self.backbone.rgbd_model.feature_6.children():
            m.apply(partial(self._nostride_dilate, dilate=self.dilate))
            self.dilate *= 2

        for m in self.backbone.hha_model.feature_6.children():
            m.apply(partial(self._nostride_dilate, dilate=self.dilate))
            self.dilate *= 2

        self.head = Head(out_planes, norm_layer, 0.001)
    ...
    def forward(self, data, hha, label=None):
        ...
        # print("the blocks size is {}".format(blocks.size()))
        # print("the merges size is {}".format(merges[0].size()))
...
class Head(nn.Module):
    def __init__(self, classify_classes, norm_act=nn.BatchNorm2d, bn_momentum=0.0003):
        ...
        self.auxlayer = _FCNHead(320, classify_classes, bn_momentum=bn_momentum, norm_layer=norm_act)
