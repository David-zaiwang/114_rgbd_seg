...
class NYUv2(BaseDataset):
    ...
    def _fetch_data(self, img_path, gt_path, hha_path, dtype=None):
        ...
        new_gt = np.zeros(shape=np.shape(gt))

        # 墙（wall）----> wall, 天花板（ceiling）----> wall
        new_gt[gt == 1] = 1
        new_gt[gt == 22] = 1
        # 地板 （floor) ----> floor
        new_gt[gt == 2] = 2
        # 窗户（window） -----> window, 百叶窗（blinds）----> window
        new_gt[gt == 9] = 3
        new_gt[gt == 13] = 3
        # 床（bed）----> bed
        new_gt[gt == 4] = 4
        # 沙发（sofa) ----> sofa
        new_gt[gt == 6] = 5
        # 柜台（counter) ----> counter, 床头柜（night stand）-----> counter
        new_gt[gt == 12] = 6
        new_gt[gt == 32] = 6
        # 椅子（chair) ------> chair
        new_gt[gt == 5] = 7
        ...
        # return img, gt, hha
        return img, new_gt.astype(np.uint8), hha
    ...
    def get_class_names(*args):
        ...
        return ['wall','floor','window', 'bed', 'sofa', 'counter', 'chair', 'table',
                'door', 'refrigerator', 'mirror', 'television', 'person']
    @classmethod
